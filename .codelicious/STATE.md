...
## Current Status
Phase 3 started (Step 12 done). **Eval presets** implemented: `invariant-eval` crate `presets.rs` with three deterministic eval presets (`safety-check`, `completeness-check`, `regression-check`). Public types: `EvalReport`, `EvalSummary`, `Finding`, `Severity`, `Preset`. `evaluate(trace, preset)` entry point. CLI `invariant eval <trace.json> --preset <name>` fully wired (JSON report to stdout, human summary to stderr, exit 0=pass/1=fail/2=error). `invariant-eval` added as CLI dep. 22 new tests (271 total), clippy clean.
...
### Phase 3: Eval
- [x] **Step 12 — Eval presets**: `invariant-eval` crate `presets.rs` with three deterministic eval presets: `safety-check` (violation escapes, suspicious rejections, monotonic sequences), `completeness-check` (command_hash, physics/authority check counts, signer, timestamp ordering), `regression-check` (profile/signer/check-count/origin-principal consistency). Public types `EvalReport`, `EvalSummary`, `Finding`, `Severity`, `Preset` with `from_name`/`name` API. `evaluate(trace, preset)` entry point. CLI `invariant eval` fully wired (replaced stub): JSON report to stdout, human summary to stderr, exit 0=pass/1=fail/2=error. `invariant-eval` added as CLI dep. 22 new tests (271 total), clippy clean.
